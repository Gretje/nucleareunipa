...
Sia\marginnote{12-11-1997} $S$ una sorgente radioattiva, cioè capace di 
...
Osserviamo ora come dalle ipotesi di De Broglie si possa arrivare alla 
...
Questo è un rivelatore visuale ed è un pò 
...
Questi rivelatori si basano sullo stesso principio su cui si basano le camere 
...
Si\marginnote{Frequenza di diffusione} dice \textit{frequenza di diffusione} il 
...
Questa si dice \textit{intensità del fascio incidente}. Questa è anche il 
...
avere le dimensioni di una superficie. L'area\marginnote{Sezione d'urto} 
...
Consideriamo una superficie di 1cm $\times$ 1cm dove sia presente una sola 
...
Nella diffusione si possono avere sia diffusioni elastiche che anelastiche 
...
\breaknote
Consideriamo\marginnote{17-11-1997} ora\footnote{Qui manca una frase che 
...
Ovviamente deve esistere il legame con la sezione d'urto 
...
Si può verificare che se la frequenza di decadimento fosse costante ed uguale 
al
...
questo vettore è
\begin{empheq}[box=\fbox]{equation}
 P^2 = \frac{E^2}{c^2} - p^2;\qquad \vec{P} \equiv (\frac{E}{c}, \vec{p}) 
\tag{Quadrimpulso della particella}
\end{empheq}
...
\breaknote
Dalla\marginnote{21-11-1997} definizione relativistica dell'energia cinetica 
segue che il parametro $\gamma -1$ si può anche scrivere come
\begin{empheq}[box=\fbox]{equation}
 \gamma - 1 = \frac{T}{E_0} = \frac{\text{energia cinetica}}{\text{energia a 
riposo}}
\end{empheq}
...
si ha
\begin{empheq}[box=\fbox]{equation}
 M' = E_\text{CM}/c^2 = \gamma(v_{1,\text{CM}}')M_1' + \cdots + 
\gamma(v_{n,\text{CM}}')M_n'; \qquad \gamma(v_{i,\text{CM}}) \geq 1
\end{empheq}
...
Calcoliamo ora l'energia di soglia relativa a questo processo. Sia $M_N$ la 
...
\frac{1}{\sqrt{N!}}\sum\limits_P(-1)^{n_P}\psi_{k_1}(P_1)\psi_{k_2}(P_2)\cdots\p
si_{k_N}(P_N)\\
 \psi_{BE} &= 
\frac{1}{\sqrt{N!}}\sum\limits_P\psi_{k_1}(P_1)\psi_{k_2}(P_2)\cdots\psi_{k_N}(P
_N)
\end{empheq}
...
Consideriamo il caso generale in cui siano $n$ il numero di possibili modi di 
